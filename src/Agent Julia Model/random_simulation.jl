...
# Must be redefined within this model (override).
agent_min = 150
agent_max = rand(Poisson(200))

# Select random number of mutants from between variables 'agent_min'
# and 'agent_max'.
initial_mutants = agent_max - rand(agent_min:agent_max)
...
mp4_path = "$(video_path)/random_simulation.mp4"
...
fig = plt.plot(
    ...
    xlims=(0, 80),
    ylims=(0, agent_max),
    title="Patient with random mutant inheritance",
    xlabel="Time (years)",
